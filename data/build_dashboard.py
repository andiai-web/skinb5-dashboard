...
def build_html(data, built_at):
    ...
    welcome_cvr  = num(['welcome series — cvr / purchase rate'], pct_decimal=True)
    welcome_rev  = num(['welcome series — revenue generated'])
    winback_open = num(['win-back flow — open rate'], pct_decimal=True)
    winback_cvr  = num(['win-back flow — cvr / purchase rate'], pct_decimal=True)
    winback_rev  = num(['win-back flow — revenue generated'])
    cart_recov   = num(['abandoned cart flow — recovery rate'], pct_decimal=True)
...
<div class="fresh">
  ...
  <span style="margin-left:auto;font-family:'DM Mono',monospace;opacity:0.6;">70 metrics · 6 sheets</span>
</div>
...
<div class="layers">
  <div class="layer l1">
    ...
    <div class="lsub">TikTok · Instagram · Substack</div>
    <div class="lrow"><span>TikTok avg views</span><span class="lval">{fn(tt_avg)}</span></div>
    <div class="lrow"><span>TikTok ER</span><span class="lval">{fp(tt_er)}</span></div>
    ...
    <div class="lrow"><span>TikTok followers</span><span class="lval">{fn(tt_followers)}</span></div>
  </div>
  <div class="layer l2">
...
<div class="sec">L1 — Social media & brand virality</div>
<div class="g5">
  <div class="kpi teal">
...
<div class="stat-grid">
  ...
  <div class="stat-card">
    <div class="stat-title"><i class="ti ti-mail"></i> Email & CRM (Klaviyo)</div>
    <div class="srow"><span class="slbl">Total subscribers</span><span class="sval">{fn(email_list)}</span></div>
    <div class="srow"><span class="slbl">Engaged subscribers</span><span class="sval">{fn(engaged_subs)} <span style="font-size:10px;color:var(--muted);">({eng_pct}% engaged)</span></span></div>
    <div class="srow"><span class="slbl">New subs this week</span><span class="sval {roas_cls(new_subs,50)}">{fn(new_subs)} <span style="font-size:10px;color:var(--muted);">(tgt +50/wk)</span></span></div>
    <div class="srow"><span class="slbl">Unsubscribe rate</span><span class="sval">{fp(unsub,2)}</span></div>
    <div class="srow"><span class="slbl">Welcome open rate</span><span class="sval">{fp(welcome_open,1)}</span></div>
    <div class="srow"><span class="slbl">Welcome CVR</span><span class="sval bad">{fp(welcome_cvr)} <span style="font-size:10px;">(tgt 3%)</span></span></div>
    <div class="srow"><span class="slbl">Welcome revenue</span><span class="sval">{fc(welcome_rev,2)}</span></div>
    <div class="srow"><span class="slbl">Win-back open rate</span><span class="sval">{fp(winback_open,1)}</span></div>
    <div class="srow"><span class="slbl">Win-back CVR</span><span class="sval bad">{fp(winback_cvr)} <span style="font-size:10px;">(tgt 8%)</span></span></div>
    <div class="srow"><span class="slbl">Win-back revenue</span><span class="sval">{fc(winback_rev,2)}</span></div>
    <div class="srow"><span class="slbl">Cart recovery rate</span><span class="sval">{fp(cart_recov,1)}</span></div>
  </div>
</div>
...
<div class="chart-card">
  ...
  <div class="cs">Meta target 2.5× · Google target 3.0×. Dashed line = minimum viable 2.5×.</div>
  <div style="display:flex;gap:12px;margin-bottom:10px;">
    ...
    <span style="font-size:11px;color:var(--muted);"><span style="width:10px;height:10px;border-radius:2px;background:#4285f4;display:inline-block;margin-right:3px;"></span>Google</span>
  </div>
...
<div class="guide">
  ...
  <div class="grow"><div class="gnum">1</div><div class="gtxt"><strong>Your specialist fills in the Excel KPI tracker</strong> — she adds this week's column (W3, W4...) across all 6 tabs: Shopify, Email, Social, Clinic, Asia, Paid Ads. Takes ~40 minutes Monday morning.</div></div>
...
new Chart(document.getElementById('roasChart'),{{type:'bar',data:{{labels:W,datasets:[
  ...
  {{label:'Google ROAS',data:GR,backgroundColor:'#4285f4',borderWidth:0,borderRadius:3}}
]}},options:{{responsive:true,maintainAspectRatio:false,
